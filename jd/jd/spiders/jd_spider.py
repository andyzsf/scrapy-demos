...
class jd_computer(CrawlSpider):
    ...
    def parse_jdcomputer(self,response):
        ...
        for site in sites:
            ...
            item['product_url']=response.url
            item['image_url']=site.xpath('//div[@id="preview"]/div/img[@src]').re(r'src="(.*?)" alt')
            ...
            item['price']=site.xpath('//div[@id="summary-price"]//strong[@id="jd-price"]/text()').extract()
            # item['meta']=site.xpath('//div[@id="itemInfo"]/div/div/text()').extract()
            item['comment_count']=site.xpath('//div[@id="comment-count"]//a/text()').extract()
