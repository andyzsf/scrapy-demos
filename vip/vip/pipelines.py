...
class MySQLStorePipeline(object):
    ...
    def _do_upsert(self, conn, item, spider):
        ...
        conn.execute("""SELECT EXISTS(
            SELECT 1 FROM vip WHERE guid = %s
        )""", (guid, ))
        ret = conn.fetchone()[0]
        ...
        if ret:
            conn.execute("""
                UPDATE vip
                SET product_url=%s, price=%s
                WHERE guid=%s
            """, (item['product_url'], item['price'], guid))
            spider.log("Item updated in db: %s %r" % (guid, item))
        else:
            conn.execute("""
                INSERT INTO vip (guid, product_url,price)
                VALUES (%s, %s, %s)
            """, (guid, item['product_url'], item['price']))
